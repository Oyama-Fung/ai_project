...
def _resolve_mime_and_suffix(file: UploadFile) -> tuple[str, str]:
    """根据UploadFile的content_type和扩展名共同判定
    （浏览器上传md时经常给application/octet-stream, 所以扩展名的优先级更高）"""
    ...
    raise ValidationError(f"不支持的文件类型：{file.filename}({mime or '未知'})。当前仅支持 PDF、DOCX、MarkDown、HTML")


# 删除允许的状态
_DELETABLE_STATUSES = frozenset({DocumentStatus.READY, DocumentStatus.FAILED, DocumentStatus.UPLOADING})
...
class DocumentService:
    ...
    async def upload(self, file: UploadFile, background_tasks: BackgroundTasks) -> Document:
        """获取文件相关信息上传到cos，然后注入Document表，最后启动后台ingest任务存入chunk表"""
    ...
    async def list_documents(
        ...
        *,
        ...
        """根据分页参数获取文档列表，可选按status过滤"""
        return await self.repo.list_paginated(page, page_size, status=status)

    async def delete(self, document_id: UUID) -> None:
        """删除数据库与cos的对应数据：
        如果先删cos再删db时候报错，落单的db数据没法处理了；
        先删db成功再删cos失败时，cos侧可以统一清理了。"""
    ...
    async def retry(self, document_id: UUID, background_tasks: BackgroundTasks) -> Document:
        ...
        # 防御性清场：理论上 failed 文档不会有 chunks，但写 chunks 阶段失败时可能会有残留
    ...
    async def list_chunks(
        ...
        """根据分页参数获取chunk列表
        （先确保document存在，否则空文档和"文档不存在"会混在一起）"""
        await self.get(document_id)
        items, total = await self.chunk_repo.list_paginated_by_document(document_id, page, page_size)
